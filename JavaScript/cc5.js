...
function solvesForQuadratic(a, b, c) {
    const discriminant = parseFloat(Math.sqrt((b * b) - (4 * a * c)))
    if (discriminant > 0 ) {
        root1 = parseFloat((-b + Math.sqrt((b * b) - (4 * a * c))) / (2 * a))
        root2 = parseFloat((-b - Math.sqrt((b * b) - (4 * a * c))) / (2 * a))
        return [root1, root2]
    } else if (discriminant == 0) {
        realRt = parseFloat(-b / (2 * a))
        return [realRt]
    } else {
        return []
    }
...
console.log(solvesForQuadratic(1, 0, 1))